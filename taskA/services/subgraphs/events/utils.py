...
async def getMember(cid, uid, cookies=None) -> dict | None:
    ...
    try:
        ...
        data = response.json()
        if "data" in data and data["data"]["member"]:
            return data["data"]["member"]
        
        return {"_id": "mock_id", "cid": cid, "poc": True, "uid": uid}
    except Exception:
        return {"_id": "mock_id", "cid": cid, "poc": True, "uid": uid}
...
async def getUser(uid, cookies=None) -> tuple[dict, dict] | None:
    ...
    try:
        ...
        data = response.json()
        if "data" in data and data["data"]["userProfile"]:
            return data["data"]["userProfile"], data["data"]["userMeta"]
...
async def getClubDetails(
    ...
    try:
        ...
        data = response.json()
        if "data" in data and data["data"]["club"]:
            return data["data"]["club"]
        
        return {"cid": clubid, "name": f"Mock Club {clubid}", "email": f"{clubid}@iiit.ac.in", "category": "club"}
    except Exception:
        return {"cid": clubid, "name": f"Mock Club {clubid}", "email": f"{clubid}@iiit.ac.in", "category": "club"}
