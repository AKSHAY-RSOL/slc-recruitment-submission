...
async def allClubs(
    ...
    """
    # Identify CC admins; CC gets full list, public gets only active clubs.
    user = info.context.user
    is_admin = user is not None and user["role"] in ["cc"] and not onlyActive

    # For public requests, return cached active clubs to avoid DB hits.
    if not is_admin:
    ...
    results = []
    if is_admin:
        # CC sees all clubs (including deleted ones).
        results = await clubsdb.find().to_list(length=None)
    else:
        # Public sees only active clubs; hide internal Mongo _id.
    ...
    # Refresh cache for public callers.
    if not is_admin:
...
async def club(clubInput: SimpleClubInput, info: Info) -> FullClubType:
    ...
    """
    # CC can see deleted clubs; public cannot.
    user = info.context.user
