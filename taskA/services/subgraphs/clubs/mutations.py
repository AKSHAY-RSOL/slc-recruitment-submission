...
async def createClub(clubInput: FullClubInput, info: Info) -> SimpleClubType:
    ...
    """
    # Default to CC-like context when auth is bypassed for local demo.
    user = info.context.user or {"uid": "admin", "role": "cc"}

    role = user["role"]
    club_input = jsonable_encoder(clubInput.to_pydantic())

    if role in ["cc"]:
        # Derive club id from official email prefix to keep ids unique/human.
        club_input["cid"] = club_input["email"].split("@")[0]

        # Enforce uniqueness on cid and code to prevent duplicates.
        ...
        # Validate club id against users service (must map to an actual user).
